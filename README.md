...
平日05:45にGemini API + Google Searchでニュースを先行調査し、06:00以降にも更新確認、5〜7分を目安に日本語原稿を作成。GitHub Pagesの `feed.json` をAlexaが07:15に読み上げる構成です。OpenAI APIは使いません。
...
```text
05:45 JST ─ 先行生成。公開済みの当日版があるか確認
               ├ 有効な当日版あり → APIを使わず終了
               └ 未更新 → Gemini検索調査 → 検索なしでJSON原稿編集
                           → 検証 → 生成物保存 → Pages配信 → 公開結果照合
                           → 公開済み原稿をGitに保存
06:00 / 06:25 / 06:45 JST ─ 公開を確認。未更新の場合だけ生成・配信
07:15 JST ─ Alexa定型アクションで読み上げ
```

- 平日は月〜金。祝日も実行します。05:45に先行生成し、06:00・06:25・06:45に回復機会を設けます。正常な当日版の再生成はしません。
...
- GitHubのscheduleは遅延や取りこぼしがあり、06:00開始・07:15までの完了を保証しません。公開リポジトリでは60日間活動がないと停止される場合があります。公開後のGit保存は履歴維持にも使いますが、Actionsの失敗通知と有効状態も確認してください。定刻性が必要な場合は、外部スケジューラから `repository_dispatch` を送ります。

### 外部スケジューラからの起動

ワークフローは `update-news` タイプの `repository_dispatch` を受け付けます。Google Apps Script、Cloud Scheduler、別のCIなどで平日05:45 JST・06:05 JST・06:25 JSTに次のGitHub APIを呼ぶと、GitHubの内蔵scheduleの遅延を補えます。正常な当日版が既にあれば、追加のGemini API呼び出しは行いません。

```text
POST https://api.github.com/repos/1p-MAKER/my-first-app/dispatches
Accept: application/vnd.github+json
Authorization: Bearer <GitHub token stored in the scheduler's secret store>
Content-Type: application/json

{"event_type":"update-news"}
```

トークンはリポジトリやスプレッドシートのセルに書かず、スケジューラのSecret保管機能だけに保存します。この設定を行うまでは、GitHub scheduleの遅延・欠落リスクは残ります。
...
GitHubと独立した補助として、このCodexタスクの監視を平日06:00・06:30・07:00・07:30に設定します。当日版がなければ進行中の実行を確認し、重複を避けてmainを手動起動、公開フィードまで照合します。正常時は通知せず復旧・失敗時に通知します。この補助はMacとCodexが稼働している必要があります。GitHubもCodexも動かなければ更新や失敗案内は配信できず、静的Pagesに前回版が残るため、定時到着を完全保証する構成ではありません。
